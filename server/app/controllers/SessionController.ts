...
export class SessionController {
  ...
  public async newSession(@Req() request: any, @Res() response: any) {
    try {
      ...
      const sessionName = request.body.session || request.query.session || request.headers.session;
      const getSessionInUsers = await SessionRepositorySave.getAllSessionByUserId(UserId);
  ...
  public async getSession(@Param('id') id: number, @Req() request: any, @Res() response: any) {
    try {
      const SessionRepositorySave = new SessionRepository();
  ...
  public async deleteSession(@Param('sessions') sessions: string, @Req() request: any, @Res() response: any) {
    try {
      if (!sessions) {
